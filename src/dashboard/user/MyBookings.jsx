...
const MyBookings = () => {
  ...
  const [ratingData, setRatingData] = useState({ show: false, decoratorId: null });
  ...
  const handleSubmitRating = async (rating) => {
    try {
      await axios.post(`/decorators/${ratingData.decoratorId}/rate`, { rating });
  ...
  {/* Pay Now (only if not cancelled) */}
  {(b.status !== 'cancelled') &&
    (b.payment.paymentStatus === "unpaid" || b.payment.paymentStatus === "initiated") && (
      <button
  ...
  {/* Cancel Booking (if pending & not cancelled) */}
  {b.status === "pending" && (
    <button
  ...
  {/* Edit Booking (only if not cancelled) */}
  {b.status !== "cancelled" && (
    <button
  ...
  {/* Rate Decorator (optional: allow even if cancelled) */}
